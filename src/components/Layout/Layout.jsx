      ...
      <header className="bg-[#F8F8F8] border-b border-[#E8E9EA] py-3 fixed top-0 w-full text-base z-50">
        <div className=" 2xl:px-[120px] md:px-[46px] px-5 md:py-3    flex items-center justify-between">
            ...
            <div className=" lg:hidden">
                ...
                className='flex items-center justify-center w-[30px] h-[30px] rounded-md bg-[#E8E9EA]' id="openMenu">
                    <img src="img/Vector.svg" alt="menu" className="w-[20px] h-[20px]" />
                </Link>
                <div 
                className={`absolute transform  duration-500 ease-in-out ${!menumobile ? ' -translate-x-0' : 'translate-x-full' }   top-0 left-0 w-full h-screen`} id="mobileMenuDiv">
                    <div
                        ...
                        <NavLink onClick={() => setMenumobile(prev => !prev ) } to="/" className="hover:animate-pulse font-semibold">Home</NavLink>
                        <NavLink onClick={() => setMenumobile(prev => !prev ) } to="/Sales" className="hover:animate-pulse font-semibold">Top sales</NavLink>
                        <NavLink onClick={() => setMenumobile(prev => !prev ) } to="/Collections" className="hover:animate-pulse font-semibold">Collections</NavLink>
                        <NavLink onClick={() => setMenumobile(prev => !prev ) } to="/Blog" className="hover:animate-pulse font-semibold">Our blog</NavLink>
                        <NavLink onClick={() => setMenumobile(prev => !prev ) } to="/About" className="hover:animate-pulse font-semibold">About us</NavLink>
                        <div className="flex items-center gap-5 mt-7">
                            <NavLink onClick={() => setMenumobile(prev => !prev ) }  to="/Sign"
                                className="border-r-2 text-[#0E1218] whitespace-nowrap border-[#0E1218] hover:animate-pulse px-[26px] py-2 font-semibold">Sign
                                up</NavLink>
                            <div className="bg-[#0E1218] text-white py-[10px] px-[26px] rounded-full">
                                <NavLink onClick={() => setMenumobile(prev => !prev ) } to="/Connect"  className="hover:animate-pulse font-semibold whitespace-nowrap">Connect
                                    Wallet</NavLink>
                            </div>
                        </div>
                    </div>
